...
function decrypt_key(deploy_key, enc_rsa_key_pth, options) {
  return __awaiter(this, void 0, void 0, function* () {
    ...
    fs.writeFileSync('passfile', `#!/bin/sh\necho ${deploy_key}`);

    const runner3 = new toolrunner.ToolRunner('chmod', ['+x', 'passfile'], options);
    yield runner3.exec();

    core.exportVariable('DISPLAY', 1);
    core.exportVariable('SSH_ASKPASS', './passfile');
    core.exportVariable('SSH_AUTH_SOCK', authSock);
    const runner4 = new toolrunner.ToolRunner('ssh-add', ['config/deploy_id_rsa'], options);
    yield runner4.exec();
  });
}
