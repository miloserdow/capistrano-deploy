...
function install_deps() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (process.platform == 'darwin') {
            ...
            let whoami_runner = new toolrunner.ToolRunner('whoami');
            yield whoami_runner.exec();
            ...
            let ver = core.getInput('ruby_version');
            if (ver) {
                let cmd1 = new toolrunner.ToolRunner('wget',
                    ['-q', 'https://get.rvm.io', '-O', 'iscript.sh']);
                yield cmd1.exec();
                let cmd1_2 = new toolrunner.ToolRunner('bash',
                    ['iscript.sh']);
                yield cmd1_2.exec();
                /*let cmd2 = new toolrunner.ToolRunner('sudo',
                    ['apt-add-repository', '-y', 'ppa:rael-gc/rvm']);
                yield cmd2.exec();
                let cmd3 = new toolrunner.ToolRunner('sudo',
                    ['apt-get', 'update']);
                yield cmd3.exec();
                let cmd4 = new toolrunner.ToolRunner('sudo',
                    ['apt-get', 'install', 'rvm']);
                yield cmd4.exec();*/
                //let cmd4_1 = new toolrunner.ToolRunner('sudo',
                //    ['usermod', '-a', 'runner', '-G', 'rvm']);
                //yield cmd4_1.exec();
                let cmd4_2 = new toolrunner.ToolRunner('source',
                    ['/home/runner/.rvm/scripts/rvm']);
                //yield cmd4_2.exec();                                       
                let cmd5 = new toolrunner.ToolRunner('rvm',
                    ['install', 'ruby-' + ver]);
                yield cmd5.exec();
                let cmd6 = new toolrunner.ToolRunner('rvm',
                    ['--default', 'use', 'ruby-'+ ver]);
                yield cmd6.exec();
            }       
        }  
